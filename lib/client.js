...
function buildRailAnchors(ctx, sessionId, session) {
  ...
  if (hasTurnOutlineCapability(session)) {
    // Prefer navigation turns so numbers match the host outline.
    let loaded = []
    if (chat !== undefined) {
      loaded = buildAnchorsFromNavigation(chat)
      if (loaded.length === 0) loaded = buildAnchorsFromOrder(chat)
    }
    return mergeAnchorsWithOutline(loaded, readTurnOutline(session))
  }
...
function injectStyle() {
  ...
  style.textContent = [
    ...
    '[data-dsh-anchors-outline][data-dsh-anchors-style="codex"] [data-dsh-anchors-item][data-unloaded] [data-dsh-anchors-tick]{width:5px;opacity:.45;}',
    ...
    '[data-dsh-anchors-outline][data-dsh-anchors-style="deepseek"] [data-dsh-anchors-item][data-unloaded] [data-dsh-anchors-tick]{width:6px;opacity:.4;}',
    '[data-dsh-anchors-outline][data-dsh-anchors-style="deepseek"] [data-dsh-anchors-item][data-unloaded] [data-dsh-anchors-label]{opacity:.55;}',
...
function mountAnchors(ctx) {
  ...
  const render = () => {
    ...
    for (const anchor of anchors) {
      ...
      item.addEventListener('pointerdown', (event) => {
        ...
        dismissWave(item)
        void navigateToAnchor(anchor, item)
      })
  ...
  const bindOutline = (session) => {
    ...
    if (face !== undefined && typeof face.subscribe === 'function') {
      currentOutlineUnsubscribe = face.subscribe(() => { render() })
    }
  }
  ...
  const followCurrent = () => {
    ...
    if (chat !== undefined && typeof chat.subscribe === 'function') {
      currentChatUnsubscribe = chat.subscribe(() => { render() })
    }
    bindOutline(session)
    render()
    maybeDrain(session, next)
  ...
  const paintActive = (key) => {
    ...
    for (const item of anchorItems.values()) {
      ...
      const active = typeof key === 'string' && chatKey === key
      if (active) {
        activeItem = item
  ...
  const syncActive = () => {
    ...
    pinnedKey = undefined
    const lastTick = [...railItems()].reverse().find((el) => el.hasAttribute('data-anchor-key'))
