...
function lockDeepseekListCap(list, cap) {
  ...
  const h = Math.round(Math.min(measureDeepseekContentHeight(list), cap))
  list.style.height = ''
...
function layoutOutline(host, outline) {
  ...
  if (getStyle() === 'deepseek') {
    ...
    const slotH = list instanceof HTMLElement ? lockDeepseekListCap(list, cap) : cap
    outline.style.top = `${Math.round(scrollRect.top + Math.max(0, (paneH - slotH) / 2))}px`
  } else {
