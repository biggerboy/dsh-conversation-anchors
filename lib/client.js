...
const en = {
  'title': 'Anchors',
  'empty': 'No messages',
  'expand': 'Expand anchors',
  'collapse': 'Collapse anchors',
  'image': 'Image',
  'you': 'You',
  'process': 'Thinking',
  'steps': '· {count} steps',
...
function injectStyle() {
  ...
  style.textContent = [
    ...
    '[data-dsh-anchors-outline] [data-dsh-anchors-body]{min-width:0;flex:1;display:flex;flex-direction:column;}',
    '[data-dsh-anchors-outline] [data-dsh-anchors-text]{min-width:0;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;}',
    '[data-dsh-anchors-outline] [data-dsh-anchors-answer]{min-width:0;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;color:var(--dsw-alias-label-tertiary,#8b8b8b);font-size:11px;line-height:15px;}',
...
function mountAnchors(ctx) {
  ...
  const render = () => {
    ...
    for (const anchor of anchors) {
      ...
      item.addEventListener('mouseenter', () => { showTip(item, anchor.answer !== '' ? `${anchor.summary}\n${anchor.answer}` : anchor.summary) })
      item.addEventListener('mouseleave', hideTip)
      item.addEventListener('focus', () => { showTip(item, anchor.answer !== '' ? `${anchor.summary}\n${anchor.answer}` : anchor.summary) })
      ...
      body.setAttribute('data-dsh-anchors-body', '')
      ...
      if (anchor.answer !== '') {
        ...
        answer.setAttribute('data-dsh-anchors-answer', '')
        answer.textContent = anchor.answer
  ...
  const applyCollapsed = (collapsed) => {
    ...
    head.setAttribute('title', collapsed ? t('expand') : t('collapse'))
    hideTip()
    if (host !== undefined) layoutOutline(host, outline)
  }
