...
/** DeepSeek panel: cap height so many turns scroll inside the card. */
const DEEPSEEK_LIST_MAX = 300
...
function injectStyle() {
  ...
  style.textContent = [
    ...
    '[data-dsh-anchors-outline][data-dsh-anchors-style="codex"] [data-dsh-anchors-list]{flex:0 1 auto;margin:auto 0;min-height:0;max-height:100%;width:36px;display:flex;flex-direction:column;overflow:hidden;pointer-events:auto;}',
    '[data-dsh-anchors-outline][data-dsh-anchors-style="codex"] [data-dsh-anchors-scroller]{flex:1 1 auto;min-height:0;max-height:100%;width:36px;display:flex;flex-direction:column;align-items:flex-start;gap:4px;padding:6px 0;overflow-y:auto;overflow-x:hidden;scrollbar-width:none;}',
...
/** Cap the DeepSeek rail slot height so ticks overflow inside the scroller. */
function lockDeepseekListCap(list, cap) {
  if (!(list instanceof HTMLElement) || !Number.isFinite(cap) || cap <= 0) return
  const outline = list.parentElement
  const n = list.querySelectorAll('[data-dsh-anchors-item]').length
  const contentH = n === 0 ? 0 : 16 + n * 26 + Math.max(0, n - 1) * 6
  const h = Math.round(Math.min(contentH, cap))
  list.style.height = ''
  if (outline instanceof HTMLElement) outline.style.height = `${h}px`
...
function layoutOutline(host, outline) {
  ...
  if (getStyle() === 'deepseek') {
    const cap = Math.round(Math.min(DEEPSEEK_LIST_MAX, Math.max(120, paneH - 24)))
    outline.style.setProperty('--dsh-anchors-ds-cap', `${cap}px`)
    ...
    outline.style.top = `${Math.round(scrollRect.top + Math.max(0, (paneH - cap) / 2))}px`
    if (list instanceof HTMLElement) lockDeepseekListCap(list, cap)
  } else {
    ...
    outline.style.width = ''
    outline.style.minWidth = ''
    outline.style.maxWidth = ''
    outline.style.removeProperty('--dsh-anchors-ds-cap')
    if (list instanceof HTMLElement) list.style.height = ''
  }
}
...
function mountAnchors(ctx) {
  ...
  const jumpToIndex = (index) => {
    ...
    if (typeof key === 'string') {
      activateAnchor(key)
      if (scrollToAnchor(key)) flashItem(item)
    }
  }
  ...
  /** True when the last turn cannot be scrolled to the top of the pane. */
  const scrollAtEnd = (scroll) => (
  ...
  const revealInList = (item, force = false) => {
    ...
    const cap = Math.round(Number.parseFloat(outline?.style.getPropertyValue('--dsh-anchors-ds-cap') || String(DEEPSEEK_LIST_MAX)))
    lockDeepseekListCap(list, cap)
  ...
  const paintActive = (key) => {
    ...
    for (const [anchorKey, item] of anchorItems) {
      const active = typeof key === 'string' && anchorKey === key
      if (active) {
        activeItem = item
  ...
  const syncActive = () => {
    ...
    const scroll = host.matches('[data-conversation-scroll]')
      ...
      : host.querySelector('[data-conversation-scroll]')
    if (!(scroll instanceof HTMLElement)) return
    if (pinnedKey !== undefined && Date.now() < pinUntil && anchorItems.has(pinnedKey)) {
      paintActive(pinnedKey)
      return
    }
    pinnedKey = undefined
    const lastTick = railItems().at(-1)?.getAttribute('data-anchor-key')
    const key = scrollAtEnd(scroll)
      ? (lastVisibleAnchorKey(scroll) ?? lastTick)
      : (owningAnchorKey(scroll, visibleAnchorKey(scroll))
        ?? lastVisibleAnchorKey(scroll)
        ?? lastTick)
    paintActive(typeof key === 'string' ? key : undefined)
  }
  ...
  scroller.addEventListener('scroll', () => { paintDeepseekBar(list) }, { passive: true })
